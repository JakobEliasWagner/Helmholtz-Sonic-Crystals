...
parser = argparse.ArgumentParser(description="Creates csv datasets for given input dir.")
...
def build_datasets(data_dir: pathlib.Path, out_dir: pathlib.Path, args):
    ...
    if args.tl:
        logger.info(f"Start building TL dataset.")
        build_transmission_loss_dataset(data_dir, out_dir)
    if args.pressure:
        logger.info(f"Start building pressure dataset.")
